...
impl<T> LinkedList<T> {
        ...
        T: PartialOrd + Clone, // 需要 Clone 以便复制值
    {
        ...
        while let (Some(a_node), Some(b_node)) = (a_current, b_current) {
            ...
            if a_val <= b_val {
                merged.add(a_val.clone()); // 克隆值以避免所有权问题
        ...
        while let Some(node) = a_current {
            ...
            a_current = unsafe { (*node.as_ptr()).next };
        }

        while let Some(node) = b_current {
            merged.add(unsafe { (*node.as_ptr()).val.clone() });
            b_current = unsafe { (*node.as_ptr()).next };
        }
